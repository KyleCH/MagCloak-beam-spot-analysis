...
# 1. / 6. : Hue range of [0, 1) or [0, 256), depending on the scale of
...
#    C2 = np.sqrt(alpha**2 + beta**2)
    ...
    # HSV saturation.
#    S_HSV = saturation_HSV(C, V, scale)
    # HSL saturation.
#    S_HSL = saturation_HSL(C, L, scale)
    # HSI saturation.
#    S_HSI = saturation_HSI(C, m, I, scale)
...
def from_RGB_wo_vect(img):
    ...
    if isinstance(img[0, 0, 0], int):
        scale = 256
    elif isinstance(img[0, 0, 0], float):
        scale = 1.
...
#        for x in range(lx):
            # Hue.
#            H[y, x] = hue(img[y, x, :], C[y, x], M[y, x], scale)
            # HSV saturation.
#            S_HSV[y, x] = saturation_HSV(C[y, x], V[y, x], scale)
            # HSL saturation.
#            S_HSL[y, x] = saturation_HSL(C[y, x], L[y, x], scale)
            # HSI saturation.
#            S_HSI[y, x] = saturation_HSI(C[y, x], m[y, x], I[y, x], scale)
...
#    H = vect_hue(img, C, M)
    # HSV saturation.
#    S_HSV = vect_saturation_HSV(C, V, scale)
    # HSL saturation.
#    S_HSL = vect_saturation_HSL(C, L, scale)
    # HSI saturation.
#    S_HSI = vect_saturation_HSI(C, m, I, scale)
...
if (hue_scale != 1. / 6.):
    ...
    def hue(pixel, C, M, scale):
        if (C == 0.):
            ...
            return (pixel[1]-pixel[2])/C % 6 * scale*hue_scale
        elif (M == pixel[1]):
            return ((pixel[2]-pixel[0])/C + 2.) * scale*hue_scale
        else:
            return ((pixel[0]-pixel[1])/C + 4.) * scale*hue_scale
...
def saturation_HSI(C, m, I, scale):
    if (C == 0.):
        return 0.
    else:
        return (1. - m/I) * scale
