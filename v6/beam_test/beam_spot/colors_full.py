...
def from_RGB_w_vect(img):
    ...
    C = M - m
    # Hue.
    H = vect_hue(img[:, :, 0], img[:, :, 1], img[:, :, 2], C, M)
...
def sclr_hue(R, G, B, C, M):
    if (C == 0):
        return chroma_zero
    elif (M == R):
        return (G-B)/C % 6 * hue_scale
    elif (M == G):
        return ((B-R)/C + 2.) * hue_scale
    else:
        return ((R-G)/C + 4.) * hue_scale

# ----------------------------------------------------------------------

vect_hue = np.vectorize(sclr_hue)
