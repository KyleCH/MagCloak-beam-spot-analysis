...
def process(dirs, fname, serial, current, params):
    ...
    plotter.profile1(
        ...
        mean1=img1.mean(axis=0),
        mean2=img2.mean(axis=0),
        maxima=img.max(axis=0),
        maxima1=img1.max(axis=0),
        maxima2=img2.max(axis=0),
        stats=stats[0],
        stats1=stats1[0],
        stats2=stats2[0],
        saveas=dirs['out']+'x1_{:04}.png'.format(serial),
        )
    plotter.profile1(
        bins=np.mgrid[roi[2]:roi[3]],
        bins2=np.mgrid[roi2[2]:roi2[3]],
        mean=img.mean(axis=1),
        mean1=img1.mean(axis=1),
        mean2=img2.mean(axis=1),
        maxima=img.max(axis=1),
        maxima1=img1.max(axis=1),
        maxima2=img2.max(axis=1),
        stats=stats[1],
        stats1=stats1[1],
        stats2=stats2[1],
        saveas=dirs['out']+'y1_{:04}.png'.format(serial),
        )
    # ------------------------------------------------------------------
    plotter.profile2(
        bins=np.mgrid[roi[0]:roi[1]],
        ...
        stats1=stats1[0],
        stats2=stats2[0],
        saveas=dirs['out']+'x2_{:04}.png'.format(serial),
        )
    plotter.profile2(
        bins=np.mgrid[roi[2]:roi[3]],
        bins2=np.mgrid[roi2[2]:roi2[3]],
        ...
        stats1=stats1[1],
        stats2=stats2[1],
        saveas=dirs['out']+'y2_{:04}.png'.format(serial),
        )
    # ------------------------------------------------------------------
    plotter.hist(
        bins=bin_edges,
